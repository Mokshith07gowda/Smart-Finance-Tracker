...
const MoneyLent = () => {
  ...
  return (
    <div className="money-lent-container">
      <div className="money-lent-header">
      ...
      <div className="stats-grid">
        <div className="stat-card total">
          ...
          <div className="stat-info">
            <p>Total Lent</p>
            <h3>{formatCurrency(stats.totalLent)}</h3>
          </div>
        </div>
      ...
      {/* Money Lent List */}
      <div className="money-lent-list">
        {moneyLentRecords.length === 0 ? (
          <div className="empty-state">
            <FiDollarSign size={64} />
            <h3>No Money Lent Records</h3>
            <p>Click "Add New" to record money you've lent</p>
          </div>
        ) : (
          moneyLentRecords.map((record) => (
            <div key={record._id} className={`money-lent-card ${record.isFullyPaid ? 'fully-paid' : ''}`}>
              ...
              <div className="card-actions">
                ...
                <button 
                  ...
                  onClick={() => handleDelete(record._id, record.lentTo)}
                >
                  <FiTrash2 /> Delete
                </button>
              </div>
      ...
      {/* Add Money Lent Modal */}
      {showAddModal && (
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Add Money Lent</h2>
      ...
      {showPaymentModal && selectedRecord && (
        <div className="modal-overlay" onClick={() => setShowPaymentModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Record Payment from {selectedRecord.lentTo}</h2>
              <button className="close-btn" onClick={() => setShowPaymentModal(false)}>×</button>
            </div>
            <div className="payment-info">
              <p><strong>Total Lent:</strong> {formatCurrency(selectedRecord.totalAmount)}</p>
            ...
            <form onSubmit={handlePayment}>
              <div className="form-group">
                <label>
                  <FiDollarSign /> Amount Paid Back ({currencySymbol})
                </label>
                <input
                  type="number"
